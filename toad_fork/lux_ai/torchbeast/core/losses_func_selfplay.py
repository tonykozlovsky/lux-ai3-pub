...
def losses_func_selfplay(rank, profiler, flags, batch, learner_outputs, teacher_output, shared_entropy_multiplier_worker, shared_entropy_multiplier_sapper, bootstrap_value, warmup, shared_pos_weight):
    ...
    with profiler.block("combine"):
        for act_space in batch["actions_GPU"].keys():
            actions = batch["actions_GPU"][act_space] # worker (17 3 1 2 16 1)
            actions_taken_mask = batch["info"]["actions_taken_GPU_CPU"][act_space] # worker (17 3 1 2 16 6) sapper (17 3 1 2 16 225)

            behavior_policy_logits = batch["policy_logits_GPU_CPU"][act_space] # worker (17 3 1 2 16 6) sapper (17 3 1 2 16 225)
            ...
            learner_policy_logits = learner_outputs["policy_logits_GPU_CPU"][act_space] # worker (17 3 1 2 16 6) sapper (17 3 1 2 16 225)
            ...
            if flags.use_old_teacher_loss:
                ...
                teacher_kl_loss = teacher_kl_loss_func(
                    learner_policy_logits,
                    teacher_output["policy_logits_GPU_CPU"][act_space],
                    any_actions_taken
                )
    ...
    with profiler.block("discounts"):
        discounts = (~batch["done_GPU_CPU"]).float() * flags.discounting # (17 3)

    with profiler.block("discounts_one"):
    ...
    values = learner_outputs["baseline_GPU"] # (17 3 2)
    ...
    if True or ("prediction"):
        ...
        masked_learner_outputs = learner_outputs['prediction_GPU_CPU']
        masked_gt = batch['info']['GPU1_available_actions_mask']['ground_truth_CPU']
