...
class LuxAIS3Env(environment.Environment):
    ...
    def step_env(
        ...
        def sap_unit(
            ...
            before_sap_shoot_energy = jnp.zeros(
                shape=(self.fixed_env_params.num_teams, self.fixed_env_params.max_units, 1),
                dtype=jnp.int16,
            )
            before_sap_recieve_energy = jnp.zeros(
                shape=(self.fixed_env_params.num_teams, self.fixed_env_params.max_units, 1),
                dtype=jnp.int16,
            )
            after_sap_shoot_energy = jnp.zeros(
                shape=(self.fixed_env_params.num_teams, self.fixed_env_params.max_units, 1),
                dtype=jnp.int16,
            )
            after_sap_recieve_energy = jnp.zeros(
                shape=(self.fixed_env_params.num_teams, self.fixed_env_params.max_units, 1),
                dtype=jnp.int16,
            )
            ...
            for t in range(self.fixed_env_params.num_teams):
                ...
                before_sap_recieve_energy = before_sap_recieve_energy.at[other_team_ids].set(all_units.energy[other_team_ids])
                ...
                after_sap_recieve_energy = after_sap_recieve_energy.at[other_team_ids].set(all_units.energy[other_team_ids])


                before_sap_shoot_energy = before_sap_shoot_energy.at[t].set(all_units.energy[t])
                ...
                after_sap_shoot_energy = after_sap_shoot_energy.at[t].set(all_units.energy[t])

            adjusted_energy_before = jnp.maximum(0, before_sap_recieve_energy)
            adjusted_energy_after = jnp.maximum(0, after_sap_recieve_energy)
            delta_energy_recieve = adjusted_energy_after - adjusted_energy_before

            delta_energy_shoot = after_sap_shoot_energy - before_sap_shoot_energy

            return all_units, delta_energy_recieve, delta_energy_shoot

        sap_unit_result = sap_unit(
        ...
        state = state.replace(
            units=sap_unit_result[0]
        )

        state = state.replace(
            delta_energy_recieve=sap_unit_result[1],
            delta_energy_shoot=sap_unit_result[2]
        )
    ...
    def get_obs(self, state: EnvState, params=None, key=None) -> EnvObs:
        ...
        for t in range(self.fixed_env_params.num_teams):
            ...
            team_obs = EnvObs(
                ...
                delta_energy_recieve=state.delta_energy_recieve,
                delta_energy_shoot=state.delta_energy_shoot,
            )
